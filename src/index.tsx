import React, {useEffect, useState} from 'react';
import ReactDOM from 'react-dom/client';
import './index.css';
import reportWebVitals from './reportWebVitals';
import {UserInfo} from "./UserInfo";
import {ChatInfo, ChatHistory} from "./Chatting/Chat";
import {ChatsComponent} from "./Chatting/ChatsComponent";
import {Message} from "./Chatting/Message";
import AuthorizationComponent from "./Signing/AuthorizationComponent";
import MainPage from "./MainPage";

import axios from 'axios';
import {AuthorizationClient, ChattingClient, SessionStateProvider, UserInfoProvider} from "./Signing/UserStateProvider";
const root = ReactDOM.createRoot(
    document.getElementById('root') as HTMLElement
);

/*const user1: UserInfo = {
    id: "1",
    imageBase64: "",
    userName: "Гусев Юра"
}

const user2: UserInfo = {
    id: "2",
    imageBase64: "",
    userName: "Кулебяка Наталья"
}

const user3: UserInfo = {
    id: "3",
    imageBase64: "",
    userName: "Толина Анатолия"
}


const message1: Message = {
    senderId: user1.id,
    text: "Как дела?",
    sendDate: new Date()
}

const message2: Message = {
    senderId: user2.id,
    text: "Нормально",
    sendDate: new Date()
}

const message3: Message = {
    senderId: user1.id,
    text: "Нормально нереально",
    sendDate: new Date()
}

const chat1: ChatInfo = {
    chatId: "123",
    imageBase64: user1.imageBase64,
    chatName: "First",
    lastSendMessage: message1
};

const chat2: ChatInfo = {
    chatId: "second chat",
    imageBase64: user2.imageBase64,
    chatName: "second",
    lastSendMessage: message2
};


const chat3: ChatInfo = {
    chatId: "third chat",
    imageBase64: user2.imageBase64,
    chatName: "third",
    lastSendMessage: message2
}

const selectedChat: ChatHistory = {
    messages: [message1, message2, message3, message3, message3, message3, message3, message3, message3, message3, message3, message3, message3, message3, message3,]

}*/

function initAxiosClient(){
    const win = window as any;
    if (win['axios'] != undefined)
        return

    axios.defaults.withCredentials = true;

    axios.interceptors.request.use(request => {
        console.log('Отправка запроса:', request.method, request.url);
        return request;
    });

    axios.interceptors.response.use(response => {
        console.log('Получен ответ:', response.status, response.data);
        return response;
    }, error => {
        console.log('Ошибка при получении ответа:', error.response.status, error.response.data);
        throw error;
    });



    win['axios'] = 'inited'
}

function initClients() {
    const win = window as any;
    if (win['clients'] === undefined) {
        win['clients'] = true;
        console.log('Init' + new Date().getSeconds());
    } else {
        console.log('Already set');
    }
}

initAxiosClient()
initClients()

const url = 'https:/localhost:7035'
const authClient = new AuthorizationClient(url);
const chattingClient = new ChattingClient(url);
const sessionProvider = new SessionStateProvider();
const userProvider = new UserInfoProvider(chattingClient);

root.render(
    <React.StrictMode>
        <div>
            <AuthorizationComponent userProvider={userProvider} sessionProvider={sessionProvider} authorizationClient={authClient}></AuthorizationComponent>
            <ChatsComponent
                 chattingClient={chattingClient}
                 userInfoProvider={userProvider}
                 sessionStateProvider={sessionProvider}>
            </ChatsComponent>
        </div>
    </React.StrictMode>
);


// If you want to start measuring performance in your app, pass a function
// to log results (for example: reportWebVitals(console.log))
// or send to an analytics endpoint. Learn more: https://bit.ly/CRA-vitals
reportWebVitals();
